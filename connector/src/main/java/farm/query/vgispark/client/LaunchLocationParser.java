...
/**
 * Tokenizes the {@code <argv>} portion of a {@code launch:<argv>} location
 * value into the argv list {@code LauncherClient} hashes and spawns — POSIX
 * shell-quote semantics, byte-for-byte matching the C++ VGI extension's own
 * {@code ParseLaunchArgv} ({@code src/vgi_launcher_internal.cpp} in the
 * {@code vgi} repo), which is what makes a Spark-launched worker shareable
 * with a DuckDB (or Trino) process pointed at the identical {@code launch:}
 * location: the resulting argv list feeds the same tuple hash on every side,
 * so they only agree on which worker to share if they tokenize the location
 * string identically. Ported from {@code vgi-trino}'s copy of this class
 * (itself dependency-free of Trino).
 *
 ...
 * launch:} is not implemented for Windows in {@code vgirpc} either.
 */
