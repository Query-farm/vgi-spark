 ...
 * regardless of its underlying Arrow field name. Ported from vgi-trino's
 * identical class.
 *
 * <p>Unlike Trino (which has {@code ColumnMetadata.setHidden(true)} for
 * exactly this kind of connector-native pseudo-column), Spark's
 * {@link org.apache.spark.sql.connector.catalog.Table} schema has no
 * per-column hidden flag — a rowid column is exposed like any other and
 * simply renamed, not hidden, from {@code SELECT *}.
 */
